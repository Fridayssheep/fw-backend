...
from app.services.services_dashboard import get_dashboard_highlights as get_dashboard_highlights_service  # 导入 dashboard 高亮业务函数。
from app.services.services_dashboard import get_dashboard_overview as get_dashboard_overview_service  # 导入 dashboard 总览业务函数。
...
def get_dashboard_overview_api(  # 定义 dashboard 总览接口处理函数。
    ...
    chart_range: Annotated[DashboardChartRange, Query(description="图表范围：day/week/month")] = DashboardChartRange.day,  # 声明图表范围参数并给默认值。
) -> DashboardOverviewResponse:  # 返回 dashboard 总览响应模型。
    return get_dashboard_overview_service(start_time, end_time, site_id, building_id, chart_range)  # 调用业务层并返回 dashboard 总览结果。
...
def get_dashboard_highlights_api(  # 定义 dashboard 高亮接口处理函数。
    ...
    start_time: Annotated[str | None, Query()] = None,  # 声明 start_time 查询参数，支持高亮按时间范围联动。
    end_time: Annotated[str | None, Query()] = None,  # 声明 end_time 查询参数，支持高亮按时间范围联动。
    site_id: Annotated[str | None, Query()] = None,  # 声明 site_id 查询参数，支持高亮按站点范围联动。
    building_id: Annotated[str | None, Query()] = None,  # 声明 building_id 查询参数，支持高亮按建筑范围联动。
    chart_range: Annotated[DashboardChartRange, Query(description="图表范围：day/week/month")] = DashboardChartRange.day,  # 声明图表范围参数并给默认值。
) -> DashboardHighlightsResponse:  # 返回 dashboard 高亮列表响应模型。
    return get_dashboard_highlights_service(limit, start_time, end_time, site_id, building_id, chart_range)  # 调用业务层并返回 dashboard 高亮结果。
