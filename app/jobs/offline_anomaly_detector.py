    ...
    with engine.connect() as conn:
        ...
        query = """
            SELECT 
                m.building_id, 
                m.meter,
                MAX(a.end_time) as last_analyzed_time
            FROM (SELECT DISTINCT building_id, meter FROM meter_readings) m
            LEFT JOIN anomaly_events a 
                   ON m.building_id = a.building_id AND m.meter = a.meter
            GROUP BY m.building_id, m.meter
...
def detect_anomalies_for_series(building_id, meter, last_analyzed_time):
    ...
    time_filter = ""
    params = {"b_id": building_id, "m_type": meter}
    
    if last_analyzed_time:
        time_filter = "AND timestamp >= %(baseline_time)s"
        # 退回 30 天作为计算上下文的基线
        params["baseline_time"] = pd.to_datetime(last_analyzed_time) - pd.Timedelta(days=30)
        
    # 1. 加载数据基线窗口
    query = f"""
        SELECT timestamp, meter_reading 
        FROM meter_readings 
        WHERE building_id = %(b_id)s AND meter = %(m_type)s {time_filter}
    ...
    # 只插入真正的新事件！(基线数据用于训练和计算，不重复插入其异常)
    def is_new_event(evt_time):
        if not last_analyzed_time:
            return True
        return pd.to_datetime(evt_time) > pd.to_datetime(last_analyzed_time)
        
    for idx, row in df[missing_mask].iterrows():
        if is_new_event(row['timestamp']):
